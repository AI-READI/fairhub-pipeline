...
def plot_time_and_spectral(
    df, visit_date, return_date, plot_ht=4, yaxis_adjusts_to_data=False
):
    ...
    if 0:  # original
        ...
        ax5.xaxis.set_major_locator(
            mdates.HourLocator(byhour=12)
        )  # looks good, unclear what part of day
        ax5.xaxis.set_major_formatter(mdates.DateFormatter("%Y-%m-%d %H:%M"))
...
def dataplot(conv_dict, output_folder):
    ...
    df["ts"] = df.apply(
        lambda x: get_datetime_from_timestr(x["ts"]), axis=1
    )  # convert to timestamp

    fig = snapshot(df, plot_ht=4, yaxis_adjusts_to_data=False)

    plname = conv_dict["participantID"] + "_snapshot.png"
    ...
    pltsname = conv_dict["participantID"] + "_time_and_spectral.png"
    if "t" in list(conv_dict.keys()):
        # these are only available with the extended version of the conv_dict
        visit_date = conv_dict["t"]["p_visit"]["visit_date"]
        return_date = conv_dict["t"]["p_visit"]["return_date"]
        # plot only works with the dates; otherwise it is empty
        fig2 = plot_time_and_spectral(
            df, visit_date, return_date
        )  # must do here because need df
        plotfile_name2 = output_folder + "/" + pltsname
        print(f"Save plot is next; output_file is {plotfile_name2}")
        try:
            fig2.savefig(plotfile_name2)
            # dataplot_dict['output_file'] = plotfile_name2
            print(f"Success; output_file is {plotfile_name2}")
            plt.close()
        except Exception as e:
            utils_plot_logger.error(f"Exception {e} when writing {plotfile_name2}")
            # dataplot_dict['output_file'] = 'None'
