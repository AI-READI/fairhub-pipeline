...
def worker(
    ...
    for file_item in file_paths:
        ...
        with tempfile.TemporaryDirectory(
            ...
            logger.debug(f"Unzipped {file_name} to {step2_folder}")
...
def pipeline(study_id: str, workers: int = 4, args: list = None):
    ...
    processed_data_output_folder = f"{study_id}/pooled-data/Maestro2-processed-parallel"
    processed_metadata_output_folder = f"{study_id}/pooled-data/Maestro2-metadata-parallel"
    ...
    for path in paths:
        ...
        if len(parts) != 4:
            logger.debug(f"Skipping {path} - Invalid format")
            continue
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Process maestro2 data files")
    parser.add_argument(
