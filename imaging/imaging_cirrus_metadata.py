...
def meta_data_save(filename, output_folder):
    ...
    if filename is not None:
        ...
        if dataset.SOPClassUID == "1.2.840.10008.5.1.4.1.1.77.1.5.1":
            ...
            color_channel_dimension = get_list_from_filename_retinal_photography(
                filename
            )[4]
        ...
        if dataset.SOPClassUID == "1.2.840.10008.5.1.4.1.1.77.1.5.4":
            ...
            if "spectralis_onh_rc_hr_oct" in filename:
                ...
                pixel_spacing = str(
                    dataset[0x52009229][0][0x00289110][0][0x00280030].value
                )
                slice_thickness = str(
            ...
            sop_instance_uid = dataset.SOPInstanceUID
            reference_instance_uid = dataset[0x52009229][0][0x00081140][0][
                0x00081155
            ].value
        ...
        if dataset.SOPClassUID == "1.2.840.10008.5.1.4.1.1.77.1.5.8":
            ...
            device = next(
                (
                    device
                    for device in ["maestro2", "triton", "cirrus"]
                    if device in file
                ),
                "unknown_device",
            )
