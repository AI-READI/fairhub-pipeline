...
# See comments in rescale_signals() for information about this factor
ADC_GAIN_FACTOR = 200


def audit_ecg_gain_and_units(meta_dict):
    ...
    for k, v in expected_value_dict.items():
        actual_val = meta_dict.get(k)
        if actual_val != v:
            error_count += 1
            conv_logger.error(
                f"Meta data {k} : {actual_val} not equal to expected value {v}"
            )

    return error_count


def rescale_signals(df, divide_by=ADC_GAIN_FACTOR):
...
def assemble_hea_comments(f, key_meta, verbose=False):
    ...
    Args:
        key_meta (dict): key meta contains a superset of the data used for the *.hea annotation file
    Returns:
...
def convert_ecg(ecg_xml_path, temp_csv_folder, output_wfdb_folder):
    ...
    Returns:
        conv_dict (dict): dictionary of key information regarding conversion, including
    ...
    conv_dict["participant_id"] = pID  # ToDo: sort out participantID and participant_id

    # audit meta data and exit if errors
    error_count = audit_ecg_gain_and_units(key_meta)
    if error_count > 0:
    ...
    temp_csv_file = os.path.join(temp_csv_folder, f"{ecg_fname_base}.csv")
    ...
    csv_to_wfdb(
        ...
        adc_gain=ADC_GAIN_FACTOR,  # Default is 200
        units="mV",
    ...
    src_dat = os.path.join(".", ecg_as_dat)
    src_hea = os.path.join(".", ecg_as_hea)

    dest_dat = os.path.join(output_wfdb_folder, ecg_as_dat)
    dest_hea = os.path.join(output_wfdb_folder, ecg_as_hea)
    ...
    conv_dict["qc_meta"] = {'data_acq_date': key_meta['data_acq_date'],
                            'data_acq_time' : key_meta['data_acq_time'],
                            'aireadi_id' : key_meta['long_id'],
                            # 'firstname' : key_meta['firstname'] -- already returned as 'participant_id'
                            }
