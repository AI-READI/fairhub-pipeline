...
def convert_to_utc(df, column_name, timezone):
    ...
    def localize_and_convert(dt_str):
        try:
            ...
            dt = datetime.strptime(dt_str, '%Y-%m-%dT%H:%M:%S')  # Adjusted to match the ISO 8601 format
...
inputExists = os.path.exists(args.input)
if not inputExists:
...
outputExists = os.path.exists(args.output.rstrip(".json"))
if not outputExists:
	print("Output does not exist. Creating directory to place output files")
	os.makedirs(args.output.rstrip(".json"))
	
###VERIFY THAT ARGS.INPUT AND ARGS.OUTPUT ARE CORRECTLY FORMATTED###
#places all properties and their values in args.required in to dictionary
props = args.required.split(sep=",")
props_dict = {}
for item in props:
	item_sep = item.split("=")
...
if ("effective_time_frame" not in props_dict) or ("event_type" not in props_dict) or ("source_device_id" not in props_dict) or ("blood_glucose" not in props_dict) or ("transmitter_time" not in props_dict) or ("transmitter_id" not in props_dict) or ("uuid" not in props_dict) or ("timezone" not in props_dict):
	print("All required values not present in third command line argument. Required arguments are 'effective_time_frame', 'event_type', 'source_device', 'blood_glucose', 'transmitter_time', 'transmitter_id', 'uuid' and 'timezone'")
	sys.exit()

if props_dict['timezone'] != 'pst' and props_dict['timezone'] != 'mst' and props_dict['timezone'] != 'cst' and props_dict['timezone'] != 'est':
...
if re.match("[\w\s\(\)_\-,\.\*\/]*\.xlsx", args.input):
	df = pd.read_excel(args.input, engine='openpyxl')
elif re.match("[\w\s\(\)_\-,\.\*\/]*\.csv", args.input):
	df = pd.read_csv(args.input)
	with open(args.input, encoding = 'utf-8') as csv_file_handler:
...
df.drop(['Index', 'Event Subtype', 'Patient Info', 'Device Info', 'Insulin Value (u)', 'Carb Value (grams)', 'Duration (hh:mm:ss)', 'Glucose Rate of Change (mg/dL/min)'], axis=1, inplace=True)

#change original excel columns to Open mHealth/IEEE equivalent
df.rename(columns={"Timestamp (YYYY-MM-DDThh:mm:ss)": "effective_time_frame", "Event Type": "event_type", "Source Device ID": "source_device_id", "Glucose Value (mg/dL)": "blood_glucose", "Transmitter Time (Long Integer)": "transmitter_time", "Transmitter ID": "transmitter_id"}, inplace=True)
...
df_data = '{"header":{"uuid":' + f'"{props_dict["uuid"]}"' + f',"creation_date_time":"{datetime_str}",' + f'"patient_id":"{patient}"' + ',"schema_id":{"namespace":"omh","name":"blood-glucose","version":3.0},"modality":"sensed","acquistion_rate":{"number_of_times":1, "time_window":{"value":5, "unit":"min"}},"external_datasheets":{"datasheet_type":"source_device","datasheet_reference":"iri-of-cgm-device"}' + f',"timezone": "{props_dict["timezone"]}"' +'}, "body":' + df.to_json(orient="records", date_format="iso", date_unit="s") + '}' 
...
for data in range(len(parsed["body"])):
	...
	parsed["body"][data]["effective_time_frame"] = {"time_interval": { "start_date_time": copy_effective_time_frame, "end_date_time": copy_effective_time_frame}}
	copy_blood_glucose = parsed["body"][data]["blood_glucose"]
	if copy_blood_glucose != "Low" and copy_blood_glucose != "High":
		parsed["body"][data]["blood_glucose"]= {"unit":"mg/dL", "value":int(copy_blood_glucose)}
	else:
		parsed["body"][data]["blood_glucose"]= {"unit":"mg/dL", "value":copy_blood_glucose}
	copy_transmitter_time = parsed["body"][data]["transmitter_time"]
	parsed["body"][data]["transmitter_time"] = {"unit":"long integer", "value":int(copy_transmitter_time)}
	
#create output JSON file
args_output_list = args.output.split('/')
json_file = args_output_list[-1]
with open(args.output.rstrip(".json") + "/" + json_file , 'w', encoding = 'utf-8') as json_file_handler:
...
df.to_csv(args.output.rstrip(".json") + "/" + json_file.rstrip(".json") + "_df_to_csv.csv")	
...
for data in range(len(parsed_copy['body'])):
	parsed_copy['body'][data]['transmitter_time'] = int(parsed_copy['body'][data]['transmitter_time'])
	if parsed_copy['body'][data]['blood_glucose'] != 'High' and parsed_copy['body'][data]['blood_glucose'] != 'Low':
		parsed_copy['body'][data]['blood_glucose'] = int(parsed_copy['body'][data]['blood_glucose'])
fieldnames = parsed_copy['body'][0].keys()
with open(args.output.rstrip(".json") + "/" + json_file.rstrip(".json") + "_standard_json_to_csv.csv", 'w') as json_convert:
	writer = csv.DictWriter(json_convert, fieldnames=fieldnames)
	writer.writeheader()
	writer.writerows(parsed_copy['body'])

###PERFORM QUALITY CONTROL TESTS###
with open(args.output.rstrip(".json") + "/" + "QC_results.txt", 'w', encoding= 'utf-8') as f:
	...
	f.write("\n")
	df.drop(['effective_time_frame', 'event_type', 'source_device_id', 'transmitter_id'], axis=1, inplace=True)
	for column in df.columns:
		...
		f.write("\n")
		QC.df_hist(df[column], args.output.rstrip(".json") + "/" + json_file.rstrip(".json") + "_input_" + column)
	...
	for data in range(len(parsed['body'])):
		effective_time_frame_list.append(parsed['body'][data]['effective_time_frame']['time_interval']['start_date_time'])
		blood_glucose_list.append(parsed['body'][data]['blood_glucose']['value'])
		transmitter_time_list.append(parsed['body'][data]['transmitter_time']['value'])
	...
	f.write("\n")
	QC.list_hist(blood_glucose_list, "Blood Glucose", args.output.rstrip(".json") + "/" + json_file.rstrip(".json"))	
	...
	QC.list_hist(transmitter_time_list, "Transmitter Time", args.output.rstrip(".json") + "/" + json_file.rstrip(".json"))
