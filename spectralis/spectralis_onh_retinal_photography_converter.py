...
spectralis = ConversionRule(
    ...
    sequences=[
        ...
        ElementList(
            ...
            "SQ",
            [
                Element("CodeValue", "00080100", "SH", HARMONIZE, "81016008"),
                Element("CodingSchemeDesignator", "00080102", "SH", HARMONIZE, "SCT"),
                Element(
                    "CodeMeaning", "00080104", "LO", HARMONIZE, "Optic nerver head"
                ),
            ],
        ),
        ElementList(
            ...
            "SQ",
            [
                Element("CodeValue", "00080100", "SH", HARMONIZE, "392001008"),
                Element("CodingSchemeDesignator", "00080102", "SH", HARMONIZE, "SCT"),
                Element(
                    "CodeMeaning",
                    "00080104",
                    "LO",
                    HARMONIZE,
                    "Scanning Laser Opththalmoscope",
                ),
            ],
        ),
        ElementList(
            ...
            "SQ",
            [
                Element("CodeValue", "00080100", "SH", HARMONIZE, "255531009"),
                Element("CodingSchemeDesignator", "00080102", "SH", HARMONIZE, "SCT"),
                Element("CodeMeaning", "00080104", "LO", HARMONIZE, "Right gaze"),
            ],
        ),
    ],
)
...
def convert_dicom(input, output):
    """
    Converts a DICOM file using a specified conversion rule and saves the converted file to the output path.

    This function reads a DICOM file from the provided input path, applies a predefined conversion rule to
    process the DICOM data, and writes the converted data to a new file in the specified output path.
    The function also handles any exceptions that occur during the conversion process and logs the conversion
    details.
    ...
    Returns:
        dict: A dictionary containing:
            - "Rule" (str): The conversion rule applied. Set to "N/A" if an error occurred.
            - "Converted" (str): Indicates whether the conversion was successful ('yes') or not ('no').
            - "Input" (str): The path of the input DICOM file.
            - "Output" (str): The path of the output DICOM file.
    """
    conversion_rule = spectralis
