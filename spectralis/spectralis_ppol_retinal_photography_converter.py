...
spectralis = ConversionRule(
    ...
    sequences=[
        ...
        ElementList(
            ...
            "SQ",
            [
                Element("CodeValue", "00080100", "SH", HARMONIZE, "408744005"),
                Element("CodingSchemeDesignator", "00080102", "SH", HARMONIZE, "SCT"),
                Element("CodeMeaning", "00080104", "LO", HARMONIZE, "Primary gaze"),
            ],
        ),
    ],
)
...
def convert_dicom(input, output):
    """
    Converts a DICOM file from the input path using a specified conversion rule and saves the converted file to the output path.

    This function reads a DICOM file from the provided input path, applies a conversion rule (defined in `conversion_rule`),
    and writes the converted DICOM data to a file in the specified output path. It handles errors by catching exceptions
    and logs the conversion status and rule applied.
    ...
    Returns:
        dict: A dictionary containing the conversion rule used, the conversion status ('yes' or 'no'),
              the input file path, and the output file path.

    Raises:
        Exception: Catches any exceptions that occur during the conversion process, setting the conversion rule to "N/A"
