    ...
    if "FitnessTracker-" in pt:
        ...
        try:
            ...
            for file_path in pt_heartrate_files:
                ...
                # Get HR file name
                hr_file_name = file_path.split("Sleep/")[1].replace(
                    "/sleep_data.csv", ""
                )
                ...
                if not sleep_df.empty:
                    ...
                    for i in range(len(sleep_df) - 1):
                        ...
                        start_time = pd.to_datetime(
                            sleep_df["adjusted_timestamp"].iloc[i]
                        )
                    ...
                    json_output["body"]["sleep"].sort(
                        key=lambda x: datetime.strptime(
                            x["sleep_stage_time_frame"]["time_interval"][
                                "start_date_time"
                            ],
                            "%Y-%m-%dT%H:%M:%SZ",
                        )
            ...
            out_directory = "sleep/garmin_vivosmart5/" + pt.replace(
                "FitnessTracker-", ""
            )
            out_directory_path = Path(
                "sleep/garmin_vivosmart5/" + pt.replace("FitnessTracker-", "")
            )
