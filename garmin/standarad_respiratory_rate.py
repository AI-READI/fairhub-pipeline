...
def merge_json_files(file_paths , outdir , ptname):
    ...
    combined_data = {
        ...
        'body': {
            'breathing': combined_body_resp_rate
        }
    }

    # Write the combined dictionary to a new JSON file
    with open(outdir + '/' + ptname + '_respiratoryrate' + '.json', 'w') as combined_file:
    ...
    if("FitnessTracker-" in pt):
        ...
        for entry in os.listdir(monitor_files):
            ...
            if os.path.isdir(full_path):
                hr_file = root_dir + pt  + "/Garmin/Monitor/" + entry + "/respiration_rate_data*.csv"
        ...
        for file_path in pt_heartrate_files:
            ...
            # Get HR file name
            hr_file_name = file_path.split("respiration_rate_data_")[1].replace(".csv" , "")
            ...
            output_dir = Path("respiratory_rate_jsons/" +patient_ID + "_respiratory_rate")
        ...
        ## Merge files:
        file_paths = []
        pt_directory = 'respiratory_rate_jsons/' + pt + "_respiratory_rate/"
        pt_directory_path = Path(pt_directory)
        out_directory = 'respiratory_rate/garmin_vivosmart5/' + pt.replace("FitnessTracker-" , "")
        out_directory_path = Path('respiratory_rate/garmin_vivosmart5/' + pt.replace("FitnessTracker-" , ""))
