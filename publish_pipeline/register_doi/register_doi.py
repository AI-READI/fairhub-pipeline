...
def create_payload(dataset_description):
    ...
    for contributor in dataset_description["contributor"]:
        ...
        split_name = contributor_name.split(",")

        if len(split_name) > 1:
            contributor_obj["familyName"] = split_name[0]
            contributor_obj["givenName"] = split_name[1]
    ...
    for creator in dataset_description["creator"]:
        ...
        split_name = creator_name.split(",")

        if len(split_name) > 1:
            creator_obj["familyName"] = split_name[0]
            creator_obj["givenName"] = split_name[1]
    ...
    for related_identifier in dataset_description["relatedIdentifier"]:
        ...
        if "relatedMetadataScheme" in related_identifier:
            related_identifier_obj["relatedMetadataScheme"] = related_identifier[
                "relatedMetadataScheme"
            ]
        if "schemeURI" in related_identifier:
            related_identifier_obj["schemeURI"] = related_identifier["schemeURI"]
        if "schemeType" in related_identifier:
            related_identifier_obj["schemeType"] = related_identifier["schemeType"]
        if "resourceTypeGeneral" in related_identifier:
            related_identifier_obj["resourceTypeGeneral"] = related_identifier[
                "resource_type"
            ]
        related_identifiers.append(related_identifier_obj)
